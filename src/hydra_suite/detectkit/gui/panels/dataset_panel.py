...
class DatasetPanel(QWidget):
    ...
    def _selected_source_level(self) -> GeometryLevel:
        """Return the currently selected source's stored geometry level.

        Falls back to OBB when no source is selected/tracked -- the same
        default used throughout this panel for an unknown level.
        """
        src_obj = self._selected_source_obj()
    ...
    def _prepare_xal_stage(self, source_dir: Path) -> Path:
        """Create (or refresh) the staging copy for X-AnyLabeling.

        Always re-copies images/labels/classes.txt from *source_dir* -- even
        if a stage from a previous launch already exists -- so that any
        DetectKit-side change made since the last launch (clearing labels,
        editing them, accepting a SAM2 escalation, ...) is reflected before
        conversion. Without this, a stale stage silently reappears every
        time X-AnyLabeling is (re)opened, regardless of what changed on the
        DetectKit side in between. Image files and label .txt files are
        overwritten in place; any xlabel JSON sidecars already staged from a
        prior conversion are untouched (they don't share a name with an
        image/label file) and get regenerated by the yolo2xlabel conversion
    ...
    def _sync_xal_stage_back(self, source_dir: Path, stage_dir: Path) -> None:
        ...
        src_obj = self._selected_source_obj()
        intended = self._selected_source_level()
    ...
    def _try_xlabel_convert(self, path: str, mode: str | None = None) -> None:
        """Attempt to convert xlabel JSON labels to YOLO format via conda env.

        *mode* selects which xlabel shape type the conversion extracts
        (detect/obb/segment, from ``xal_mode_for_level``) and must match the
        source's actual geometry level -- e.g. a source escalated to segment
        (polygon) needs mode "segment", or its polygon annotations edited in
        X-AnyLabeling are not the shape type the converter looks for and
        come back empty/wrong. Defaults to the currently selected source's
        stored level when not given explicitly.
        """
        if mode is None:
            mode = xal_mode_for_level(self._selected_source_level())
        env = self._selected_xal_env()
        try:
            from hydra_suite.integrations.xanylabeling.cli import convert_project

            ok, msg = convert_project(path, path, conda_env=env, mode=mode)
            if ok:

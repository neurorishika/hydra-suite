...
_START_GATE_FD_ENV = "HYDRA_START_GATE_FD"
_PR_SET_PDEATHSIG = 1
...
def main(argv: Sequence[str] | None = None) -> int:
    """Wait for containment, apply limits, and replace this process."""
    ...
    start_gate_fd = os.environ.pop(_START_GATE_FD_ENV, None)
    if start_gate_fd is not None:
        descriptor = int(start_gate_fd)
        try:
            start_message = os.read(descriptor, 1)
        finally:
            os.close(descriptor)
        if start_message != b"G":
            raise RuntimeError("supervisor exited before containment became ready")
    apply_child_limits(

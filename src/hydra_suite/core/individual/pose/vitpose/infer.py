...
def decode_and_project(
    ...
    heatmaps: (B, K, geom.heatmap_size_wh[1], geom.heatmap_size_wh[0]).
    centers/scales: (B, 2). Returns coords (B, K, 2) in image space and
    maxvals (B, K, 1).
    """
    ...
    for i in range(coords.shape[0]):
        out[i] = transform_preds(coords[i], centers[i], scales[i], geom.heatmap_size_wh)
    return out, maxvals

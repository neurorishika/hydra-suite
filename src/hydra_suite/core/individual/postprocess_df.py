...
def _annotate_identity_summary_columns(df: pd.DataFrame, params) -> pd.DataFrame:
    ...
    def _sources_and_conflict_columns(
        ...
        for token, flag in (
            ...
            addition = np.where(flag_arr, np.where(have_any, "," + token, token), "")
...
def _mirror_realtime_and_tag_into_final(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if C.REALTIME_LABEL in out.columns:
        ...
        if has_realtime.any():
            ...
            if C.REALTIME_ID in out.columns:
                out.loc[has_realtime, C.FINAL_ID] = out.loc[has_realtime, C.REALTIME_ID]
    ...
    if "DetectedTagLabel" in out.columns and empty_final.any():
        ...
        has_tag = (
            empty_final & tag_label.notna() & (tag_label.astype(str).str.strip() != "")
        )
        if has_tag.any():
...
def derive_identity_keys(df: pd.DataFrame, params) -> pd.DataFrame:
    """Annotate identity-evidence summary columns and ``UniqueIdentityKey``.

    This is the ``_annotate_identity_summary_columns`` + ``UniqueIdentityKey``
    tail of the original identity post-processing pipeline, split out so it
    can run standalone -- BEFORE the solver, so the relink step (which reads
    ``UniqueIdentityKey`` to match fragments) always sees keys derived from
    the CURRENT frame, and again AFTER the solver (inside
    :func:`resolve_identity`) once the final structure is known.
    """
    if df is None or df.empty:
        return df

    with_pose_df = _annotate_identity_summary_columns(df, params)
    try:
        from hydra_suite.core.individual.identity.heads import (
            HEADS_UNKNOWN,
...
def resolve_identity(
    with_pose_df: pd.DataFrame, params, identity_evidence_cache_path=None
) -> pd.DataFrame:
    """Run the fragment solver exactly once and finalize the Final family.

    Pipeline (order is load-bearing, per the design-spec adversarial review):
    solver -> ``_stamp_non_identifying_labels`` ->
    ``_mirror_realtime_and_tag_into_final`` ->
    ``fill_identity_nans_with_consensus`` -> ``sort_trajectories_by_identity``
    -> :func:`derive_identity_keys` (re-derive ``IdentityEvidence*``/
    ``UniqueIdentityKey`` after the final structure -- including any
    solver-assigned labels and any relink-driven ``TrajectoryID``
    renumbering upstream of this call -- is known).
    ...
    return derive_identity_keys(with_pose_df, params)
...
def apply_identity_postprocessing_to_df(
    ...
    Backward-compatible composition of the two split stages: derive the
    (pre-solve) identity keys, then resolve identity (solver + finalization,
    which re-derives the keys once more at the end). Kept for existing
    callers/tests that invoke identity post-processing as one call; new
    callers that need the relink step to run BETWEEN key-derivation and
    resolution (Task 6: relink before identity resolution) should call
    :func:`derive_identity_keys` and :func:`resolve_identity` directly.
    """
    if with_pose_df is None or with_pose_df.empty:
        return with_pose_df
    return resolve_identity(
        derive_identity_keys(with_pose_df, params),
        params,
        identity_evidence_cache_path,
    )

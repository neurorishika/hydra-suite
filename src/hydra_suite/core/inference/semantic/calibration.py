...
# The matcher geometry (representative-point fix, containment gate,
# quality-ranked greedy pairing) moved to core/inference/match_geometry.py
# (2026-09-06) so the direct-calibration path can share it without importing
# under core/inference/semantic/. Re-exported here (noqa: F401 -- these are
# used by external importers, e.g. tools/sam3_parity/matcher_gate.py, that
# still reach them via this module), same objects, so every existing
# importer of this module keeps working -- see
# tests/test_shared_scoring_primitives_identity.py.
from hydra_suite.core.inference.match_geometry import (  # noqa: F401
    _contains,
    _is_finite,

...
class IdentityPanel(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.spin_identity_offline_split_min_conf.setToolTip(
            "Minimum smoothed posterior confidence required before offline identity splitting considers a label regime stable."
        )
        ...
        self.spin_identity_offline_split_min_margin.setToolTip(
            "Required posterior margin between the top two labels before offline splitting treats a regime change as trustworthy."
        )
        ...
        self.spin_identity_offline_split_min_frames.setToolTip(
            "Minimum consecutive frames needed on each side of an offline identity switch before the trajectory is split."
        )
        ...
        self.spin_identity_offline_split_max_bridge_frames.setToolTip(
            "Maximum ambiguous bridge length tolerated between two strong offline identity regimes before splitting is suppressed."
        )
        ...
        fl_identity_decoder.addRow(
            "Offline split confidence",
            self._build_inline_fields_row(
                [
                    ("Min conf", self.spin_identity_offline_split_min_conf, 0),
                    ("Min margin", self.spin_identity_offline_split_min_margin, 0),
                ]
            ),
        )
        fl_identity_decoder.addRow(
            ...
            self._build_inline_fields_row(
                [
                    ("Min frames", self.spin_identity_offline_split_min_frames, 0),
                    (
                        "Max bridge",
        ...
        fl_identity_decoder.addRow(
            "Offline global solver",
            self._build_inline_fields_row(
                [
                    ("Time limit (s)", self.spin_identity_offline_ilp_time_limit, 0),
                    ("Rel gap", self.spin_identity_offline_ilp_rel_gap, 0),
                ]
            ),
        )
        fl_identity_decoder.addRow(
            ...
            self._build_inline_fields_row(
                [
                    (
                        "Strength",
                        self.spin_identity_respawn_prior_strength,
                        0,
                    ),
                    ("Decay", self.spin_identity_respawn_prior_decay, 0),
                    ("Max gap", self.spin_identity_respawn_prior_max_gap, 0),
                ]
            ),
        )
        identity_content_layout.addWidget(self.g_identity_decoder_tuning)
    ...
    def _sync_realtime_individual_batch_ui(self) -> None:
        ...
        self.lbl_individual_batch_notice.setText("\n".join(messages))

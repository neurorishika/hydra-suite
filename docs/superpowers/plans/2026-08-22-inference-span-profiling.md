...
- **Implementation rule 2:** every hunk in the **instrumentation** commits (Tasks 9-11) is a `with span(...)` / `@spanned` wrapper, an import, or an indentation change from one of those. No logic edits. Two sanctioned exceptions live in earlier, separately-reviewable commits: Task 6's `_effective_depth` depth clamp (behavior change, but only under `HYDRA_PROFILE_GPU=1`) and Task 7's deletion of the `HYDRA_RT_PROFILE` machinery. A third sanctioned exception: Task 9's fix round restructured `pipeline.py`'s `_stream_windows` (a genuine, disclosed, output-equivalent restructure to make the DECODE span placement correct), not a pure wrapper insertion.
- **Implementation rule 3:** spans wrap loops, never loop bodies. The "no measurable cost" claim depends on it. Sanctioned per-frame exception: `READ` (`frame_prefetcher.py`, all three prefetcher classes) and `FLUSH` (`cache/writer.py`) deliberately open one span per frame inside a loop body, because wrapping from outside the loop would measure queue-wait instead of per-frame decode/flush cost.

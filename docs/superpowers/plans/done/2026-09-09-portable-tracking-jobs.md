...
| Full matrix (Task 4 complete) | CUDA (firebrat) | `692f5f7f` vs base `8ec4730d` | **PASS** | **9 clips, 54 EQUIVALENT / 0 divergent.** PERF 0.99–1.01x (tol 1.25). Row counts substantial (ant_obb_sleap 11023, ant_cnn_identity_relink 10172, ant_cnn_identity 10156, ant_pose_headtail 7870, worm_bgsub 2725, ant_obb_sequential 1366, fly_obb 1393, fly_obb_roi 1393, worm_bgsub_scaled 1061). SLEAP genuinely ran: 24 populated `PoseKpt_*` columns. Box was idle and clean (no stale sleap/hydra procs). Branch transported by git bundle from common ancestor `01c1bb5e` — firebrat lacks `8f9688e0` because local main is ahead of origin. |
...
| Final matrix | CUDA (firebrat) | — | (covered by the row above) | The Task-4 CUDA matrix above was run at the branch tip `692f5f7f`, which already contains Tasks 1–4. Re-run after Tasks 9–12 land. |
| Full pytest delta | MPS | `f6147629` vs base `8ec4730d` | **PASS — zero regressions** | **9464 passed, 17 failed, 24 skipped.** Every failure accounted for against the baseline, comparing SETS not counts: **14 fail identically on the baseline worktree** (al_inference_adapter, classkit_synthetic_generalization x2, gpu_utils_numba_fallback sam3-sidecar, gui_session_cutover_equivalence, obb_coreml_export x3, postproc_identity_gating x2, sam3_publish_sidecar torch-import purity, trackerkit_cli_cutover x2, worker_real_inference_integration); **2 are the documented order-dependent `vitpose_export` flakes** (both pass in isolation on the branch); **1 is a load-sensitive timing flake** (`utils/test_profiling::test_nesting_and_self_time` — failed only while the baseline suite ran concurrently; passes 3/3 idle on BOTH trees, 0.08s idle vs 3.65s loaded). TRAP NOTE: the two `trackerkit_cli_cutover` and the `gui_session_cutover_equivalence` failures SKIP in a fixture-less worktree, which fakes a regression — the baseline had to have the fixture clips symlinked in before the comparison was meaningful. |
| Round trip + cache hit | Mac → firebrat → Mac | `bbdeccb8` | **PASS (Goal 4 proven)** | `job pack` (fly_obb) → `push` (remote verify ok) → `run` on firebrat → `pull`. **Zero registration:** firebrat had NO host model registry at all (`no host registry`) and the run still loaded every model from `<job>/models`. Tracking completed on the box: `fly_obb_tracking_final.csv`, 3 trajectories, 487 fps, plus an active-learning dataset export. `pull` placed every artifact beside the ORIGINAL clip incl. `.inference_cache_fly_obb/`. **Cache-key identity across machines:** the CUDA-produced cache stores `v5\|sha256:b5047e47…\|d80121cc…` and a LOCAL Mac run computes the byte-identical string — same model content, different absolute path, same key. Under the old `(path, mtime)` key this was impossible by construction. |

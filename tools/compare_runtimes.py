...
TRT_PATH  = str(next(MODELS_DIR.glob("obb/20260423-172251_26x_obiroi_train36_b1.engine")))

class_names = {0: "fish"}
nc = 1

print("Creating CUDA executor …")
cuda_exec = DirectPyTorchCUDAOBBExecutor(pt_model, IMGSZ, class_names=class_names, class_count=nc)
print("Creating ONNX executor …")
onnx_exec = DirectONNXOBBExecutor(ONNX_PATH, IMGSZ, class_names=class_names, class_count=nc)
print("Creating TensorRT executor …")
trt_exec  = DirectTensorRTOBBExecutor(TRT_PATH, IMGSZ, class_names=class_names, class_count=nc)

print(f"\nExecutor info:")
print(f"  CUDA: _end2end={cuda_exec._end2end}")
print(f"  ONNX: _end2end={getattr(onnx_exec, '_end2end', False)}, fp16={onnx_exec._fp16}, "
      f"static_out={onnx_exec._static_out_shape}")
print(f"  TRT:  _end2end={trt_exec._end2end}, static_out={getattr(trt_exec, '_static_out_shape', 'N/A')}")
...
def run_exec(executor, frame: np.ndarray, conf_thres: float) -> np.ndarray:
    ...
    results = executor.predict([frame], conf_thres=conf_thres, classes=None, max_det=MAX_DET)
    r = results[0]
...
def apply_size_ar_filter(data: np.ndarray) -> np.ndarray:
    ...
    keep = (
        (ellipse_area >= MIN_SIZE) & (ellipse_area <= MAX_SIZE) &
        (ar >= MIN_AR) & (ar <= MAX_AR)
    )
    return data[keep]
...
# ─── Comparison helper ────────────────────────────────────────────────────────
def compare(a_name, a_data, b_name, b_data, tol_px=1.5, tol_conf=0.02, tol_angle=0.05, top_n=5):
    if len(a_data) != len(b_data):
        ...
        if len(a_data) > 0 and len(b_data) > 0:
            ...
            for row in a_data[:n_show]:
                print(f"       cx={row[0]:.1f} cy={row[1]:.1f} w={row[2]:.1f} h={row[3]:.1f} "
    ...
    ok = (max_pos <= tol_px and max_wh <= tol_px and max_ang <= tol_angle and max_conf <= tol_conf)
    sym = "✓" if ok else "✗"
    print(f"  {sym} {a_name} vs {b_name}: "
...
print("\n" + "=" * 70)
print(f"PHASE 2: Detections at conf_thres={CONF_THRES} (working threshold, no size/AR filter)")
print("=" * 70)
...
print(f"Phase 2 conf-thresh dets match (no filter): {'✓ YES' if all_ok_conf else '✗ NO'}")
print(f"Phase 3 filtered dets match:                {'✓ YES' if all_ok_filt else '✗ NO'}")

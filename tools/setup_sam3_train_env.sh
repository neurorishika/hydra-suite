...
conda run -n "$SAM3_ENV_NAME" pip install einops torchmetrics scipy decord iopath \
...
echo "Installing hydra-suite editable from $REPO_ROOT..."
conda run -n "$SAM3_ENV_NAME" pip install -e "$REPO_ROOT"

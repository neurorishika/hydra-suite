...
**Labelled/unknown split and label-vs-evidence agreement, reported
honestly:** 102 of 201 trajectories (11828 of 15867 rows, ~74.5% of rows)
carry a real identity label; 99 trajectories (4039 rows, ~25.5%) are
`"unknown"`. Every one of the 102 labelled trajectories' `IdentityFinalLabel`
...
accuracy or the CNN identity model — a 25.5% "unknown" rate on this clip
reflects the underlying evidence quality (see memory
...
| `ant_pose_headtail` | 0.000e+00 | 0.000e+00 | `forward` "missing on one side" (legacy); `final`/`final_with_individual` θ mean 0.490 rad, 10/9096 vs 7/9096 unmatched rows | Same θ-noise-floor pattern. The 10-vs-7 unmatched-row delta is consistent with Task 5's leading/trailing position-less-row trimming (this clip's config sets `interpolation_method=none`, which still runs `trim_positionless_ends` per Task 5's design) |
...
Performance: every clip's `new/legacy` wall-clock ratio was within tolerance (0.60x-1.09x, well under the 1.25x cap) — the double-solve fix (Task 6) shows up as the largest win (`ant_pose_headtail` 0.60x, though that clip has no identity solving; the ratio there is dominated by the trim/densify pass being cheap).
...
Same command/matrix as MPS, on `rutalab@mehek.taild08eb9.ts.net`, `RUNTIME=cuda`, `hydra-cuda` conda env, against the same `legacy/main` @ `157e1ae3`. Same `__pycache__`-clearing precaution applied preemptively to both worktrees before the run (the numba JIT-cache gotcha found on MPS).

...
def test_manual_timeline_reassign_moves_non_overlapping_track(tmp_path: Path) -> None:
    df = pd.DataFrame(
        {
...
def test_manual_timeline_reassign_blocks_overlapping_track(tmp_path: Path) -> None:
    df = pd.DataFrame(
        {
            "FrameID": [0, 1, 2, 2, 3, 4],
            "TrajectoryID": [1, 1, 1, 2, 2, 2],

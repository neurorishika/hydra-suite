...
def test_one_preview_per_geometry_and_merge_setting(monkeypatch, tmp_path):
    ...
    merges = (
        MergeSettings("greedy_nmm", "ios", 0.3),
        MergeSettings("greedy_nmm", "ios", 0.5),
        MergeSettings("greedy_nmm", "ios", 0.7),
    )
...
def test_points_carry_a_stable_candidate_identity(monkeypatch, tmp_path):
    from hydra_suite.detectkit.jobs import direct_calibration as job
    ...
    request = _request(tmp_path)
    outcome = job.run_direct_calibration(request)
    assert {p.candidate_index for p in outcome.points} == set(
...
def test_preview_round_trip_keeps_identity_and_per_detection_confidence(tmp_path):
    ...
    preview = job.CalibrationPreview(
        candidate_label="T",
        frames=[
            (
                image,
    ...
    job.save_direct_calibration(evidence_dir, outcome, request)

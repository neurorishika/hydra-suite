"""Regression guard (Deviation C): `_model_input_wh` must use a backend's true
(W, H) input via `preferred_input_wh`, not collapse it to a square via the
scalar `preferred_input_size` (== max(W, H)).

Bug: `_model_input_wh` in `core/inference/stages/pose.py` only read the scalar
...
`_model_input_wh` uses it directly when present. Backends with no fixed input
(SLEAP service, `preferred_input_size == 0`) must keep getting the identity
fit -- `_model_input_wh` returns `geometry.canvas_wh` for them, unchanged.
"""
...
from hydra_suite.core.inference.stages.pose import PoseModel, _model_input_wh
...
def test_vitpose_model_input_wh_is_true_non_square(tmp_path):
    ...
    wh = _model_input_wh(model, _GEOMETRY)

    assert wh == (192, 256)
    assert wh[0] != wh[1]  # NOT collapsed to a square


def test_sleap_exported_model_input_wh_is_true_non_square():
...
def test_backend_with_no_fixed_input_keeps_identity_fit_to_geometry_canvas():
    ...
    must be unaffected: `_model_input_wh` falls back to the canonical

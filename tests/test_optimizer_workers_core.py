...
def test_core_tracking_optimization_imports_no_qt():
    """Guard: the entire ``core/tracking/optimization/`` package must be
    Qt-free. Both ``optimizer_workers.py`` and ``optimizer.py`` have been
    migrated to pure cores (their QThread wrappers now live under
    ``trackerkit/gui/workers/``), so no module in this package may import Qt.
    """
    import ast
    ...
    pkg_dir = Path(opt_pkg.__file__).parent
    offenders = []
    ...
    assert (
        not offenders
    ), "core/tracking/optimization/ must not import Qt: " + "; ".join(offenders)

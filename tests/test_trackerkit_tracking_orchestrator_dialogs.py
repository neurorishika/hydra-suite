...
def test_apply_benchmark_recommendations_sets_tier_directly_without_migration(
    monkeypatch,
) -> None:
    """`detection.runtime` from a benchmark recommendation is already a tier
    string ("cpu"/"gpu"/"gpu_fast"). Applying a "gpu_fast" recommendation must
    set the tier combo to "gpu_fast", not silently reset it to "cpu" by
    routing it through the legacy `migrate_runtime_to_tier` (which only
    understands old canonical-runtime strings)."""

    class FakeCombo:
        def __init__(self) -> None:
            self.current_index = -1
            self._data = {0: "cpu", 1: "gpu", 2: "gpu_fast"}

        def findData(self, value):
            for idx, data in self._data.items():
                if data == value:
                    return idx
            return -1
        ...
        def currentData(self):
            return self._data.get(self.current_index)
    ...
    tier_combo = FakeCombo()
    detection_batch_spin = FakeSpin()

    main_window = SimpleNamespace(
        _current_pose_backend_key=lambda: "yolo",
    )
    panels = SimpleNamespace(
        ...
        identity=SimpleNamespace(
            _sync_realtime_individual_batch_ui=lambda: None,
        ),
    )
    orchestrator = ConfigOrchestrator(
        main_window=main_window,
    ...
    recommendations = {
        "detection_direct": SimpleNamespace(runtime="gpu_fast", batch_size=8),
    }

    notes = orchestrator._apply_benchmark_recommendations(recommendations)

    assert tier_combo.currentData() == "gpu_fast"
    assert detection_batch_spin.value_set == 8
    assert notes == []

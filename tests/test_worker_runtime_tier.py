...
This was previously a silent gap — `_build_inference_config_from_params` built
...
The builder only reads `params`, so it is called unbound on a bare stub to avoid
...
_BUILD = TrackingWorker._build_inference_config_from_params
...
def _tier(params: dict) -> str:
    ...
    return _BUILD(_Stub(), params).runtime_tier
